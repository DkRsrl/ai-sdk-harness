...
import { isTool, type Tool } from "./tool"
  ...
  return {
    ...
    execute: (path, args) =>
      Effect.gen(function* () {
        ...
        // Models keep writing `tools.search(...)` for the discovery global;
        // honor the spelling when no registered tool claims the name.
        const tool =
          name === "search" && root.children.get("search")?.tool === undefined
            ? searchTool
            : resolve(root, path)
