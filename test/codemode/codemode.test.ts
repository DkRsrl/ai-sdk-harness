...
import { CodeMode, Tool, toolError } from "../../src/codemode/index"
...
describe("CodeMode public contract", () => {
  ...
  test("honors tools.search as the discovery built-in when no tool claims the name", async () => {
    const runtime = CodeMode.make({ tools })

    const viaTools = await Effect.runPromise(runtime.execute(`return tools.search({ query: "order" })`))
    const viaGlobal = await Effect.runPromise(runtime.execute(`return search({ query: "order" })`))
    expect(viaTools.ok).toBe(true)
    if (viaTools.ok && viaGlobal.ok) expect(viaTools.value).toStrictEqual(viaGlobal.value)
  })

  test("a registered tool named search wins over the discovery alias", async () => {
    const search = Tool.make({
      description: "Domain search",
      input: Schema.Struct({ query: Schema.String }),
      output: Schema.String,
      execute: () => Effect.succeed("domain"),
    })
    const runtime = CodeMode.make({ tools: { search } })

    const result = await Effect.runPromise(runtime.execute(`return tools.search({ query: "x" })`))
    expect(result).toMatchObject({ ok: true, value: "domain" })
  })
